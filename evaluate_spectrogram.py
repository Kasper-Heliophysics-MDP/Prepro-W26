...
# Flags time steps with unusually high signal intensity 
...
# Identifies which frequencies contain presistent bands 
...
# Masks all but the strongest burst pixels 
...
# Measures change in signal intensity after processing in burst regions 
...
# Computes signal-to-noise ratio and related metrics 
...
# Measures decrease in persistent band intensity after processing in non-burst regions
...
# Determines in a condition passes or fails requirement
...
def evaluate_spectrogram(S):
    ...
    signal_before, signal_after, snr_before, snr_after, snr_improvement, noise_after = compute_snr_metrics(
        S, S_clean, burst_mask, persistent_rows, valid_cols,)
